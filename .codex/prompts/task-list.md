...
1. 인자(`$ARGUMENTS`)가 비어 있으면 기본 제한 50개로 조회:
   - `python3 scripts/task_commands.py list --json --limit 50`
2. 인자가 숫자면 limit으로 간주:
   - `python3 scripts/task_commands.py list --json --limit <숫자>`
3. 인자가 숫자가 아니면 검색어(keyword)로 간주:
   - `python3 scripts/task_commands.py list --json --keyword "<인자>" --limit 50`
