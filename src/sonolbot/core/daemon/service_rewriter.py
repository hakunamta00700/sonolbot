...
class DaemonServiceRewriterMixin:
    ...
    def _build_agent_rewriter_input(self, chat_id: int, state: dict[str, Any], raw_text: str) -> str:
        ...
        lines = [
            "?„ë˜ ?ë¬¸ ?ˆë‚´ë¥?ì§€ì¹¨ì— ë§ëŠ” ?¬ìš©??ì§„í–‰ ?ˆë‚´ë¬¸ìœ¼ë¡??¬ì‘?±í•˜??",
            f"- ìµœê·¼ ?¬ìš©???”ì²­: {user_hint or '(?†ìŒ)'}",
            f"- ? íƒ??TASK ?ë³„?? {selected_task_id or '(?†ìŒ)'}",
            "- ì¶œë ¥ ?•ì‹: ?”ë ˆê·¸ë¨ HTML ?Œì‹± ê¸°ì?(?„ìš” ??<b>, <code>ë§?ìµœì†Œ ?¬ìš©, Markdown ë¬¸ë²• ê¸ˆì?)",
            "- ?ë¬¸ ?ˆë‚´:",
            _service_utils.compact_prompt_text(raw_text, max_len=1200) or "(ë¹„ì–´ ?ˆìŒ)",
            "",
            "ê²°ê³¼ ë¬¸ì¥ë§?ì¶œë ¥?˜ë¼.",
        ]
        return "\n".join(lines).strip()
    ...
    def _build_agent_rewriter_fallback(self, chat_id: int, state: dict[str, Any], raw_text: str) -> str:
        ...
        if user_hint:
            return (
                f"?”ì²­?˜ì‹  '{user_hint}' ?´ìš©?????•í™•???¤ëª…?œë¦¬ê¸??„í•´ ê´€???´ìš©???•ì¸?˜ê³  ?µì‹¬???•ë¦¬?˜ëŠ” ì¤‘ì…?ˆë‹¤. "
                "? ì‹œë§?ê¸°ë‹¤??ì£¼ì‹œë©??´í•´?˜ê¸° ?½ê²Œ ?´ì–´???ˆë‚´?œë¦´ê²Œìš”."
            )
        return (
            "?”ì²­?˜ì‹  ?´ìš©???•í™•?˜ê²Œ ?¤ëª…?œë¦¬ê¸??„í•´ ê´€???´ìš©???•ì¸?˜ê³  ?µì‹¬???•ë¦¬?˜ëŠ” ì¤‘ì…?ˆë‹¤. "
            "? ì‹œë§?ê¸°ë‹¤??ì£¼ì‹œë©??´í•´?˜ê¸° ?½ê²Œ ?´ì–´???ˆë‚´?œë¦´ê²Œìš”."
        )
    ...
    def _normalize_agent_rewriter_output(self, text: str) -> str:
        ...
        if not normalized:
            return ""
        normalized = re.sub(r"^\s*(?¬ì‘??ê²°ê³¼[:ï¼?\s*)", "", normalized, flags=re.IGNORECASE)
